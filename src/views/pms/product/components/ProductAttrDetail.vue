<template>
  <div style="margin-top: 50px">
    <el-form :model="value" ref="productAttrForm" label-width="100px" size="small">
        ...
        <el-table style="width: 100%;margin-top: 20px"
          ...
          <!-- 10.26新增需求 商品积分-->
            <el-table-column
              ...
              align="center">
              <template slot-scope="scope">
                <el-input v-model="scope.row.giftPoints == null ? scope.row.giftPoints = 0 : scope.row.giftPoints"></el-input>
              </template>
            </el-table-column>
          <div v-if="value.verifyStatus">
            <el-table-column
              label="兑换积分"
              align="center">
              <template slot-scope="scope">
...
<!--      </el-form-item>-->
      ...
      <el-form-item label="商品视频：">
        <single-upload-vedio v-model="value.brandName"></single-upload-vedio>
      </el-form-item>
      <el-form-item label="商品详情图片（详情图请按照设计排序添加）：">
        <multi-upload v-model="selectProductDetailPics"></multi-upload>
      </el-form-item>
...
<script>
  ...
  import SingleUploadVedio from '@/components/Upload/singleUploadOnlyVedio'
  import MultiUpload from '@/components/Upload/multiUpload'
  import Tinymce from '@/components/Tinymce'

  export default {
    name: "ProductAttrDetail",
    components: {SingleUpload, MultiUpload, Tinymce, SingleUploadVedio},
    props: {
      value: Object,
      isEdit: {
        type: Boolean,
    ...
    computed: {
      ...
      selectProductDetailPics:{
        get:function () {
          let pics1=[];
          if(this.value.albumDetailPics===undefined||this.value.albumDetailPics==null||this.value.albumDetailPics===''){
            return pics1;
          }
          let albumDetailPics = this.value.albumDetailPics.split(',');
          for(let i=0;i<albumDetailPics.length;i++){
            pics1.push(albumDetailPics[i]);
          }
          return pics1;
        },
        set:function (newValue) {
          if (newValue == null || newValue.length === 0) {
            ...
            this.value.albumDetailPics = null;
            this.value.albumDetailPics = newValue[0];
            if (newValue.length > 1) {
              this.value.albumDetailPics = this.value.albumDetailPics + ',';
            }
            if (newValue.length > 1) {
              for (let i = 1; i < newValue.length; i++) {
    ...
    methods: {
      ...
      getProductAttrList(type, cid) {
        let param = {pageNum: 1, pageSize: 1000, type: type};
      ...
      mergeProductAttrPics() {
        for (let i = 0; i < this.selectProductAttrPics.length; i++) {
          console.log(this.selectProductAttrPics.length)
